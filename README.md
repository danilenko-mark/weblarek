...
Класс отвечает за хранение каталога товаров и выбранного товара для подробного просмотра.
...
`setProducts(products: IProduct[]): void` - сохраняет массив товаров в модель каталога.
...
`setData(data: Partial<IBuyer>): void` - сохраняет данные покупателя и позволяет изменять отдельные поля без удаления остальных данных.
`getData(): IBuyer` - возвращает все данные покупателя.
`clear(): void` - очищает данные покупателя.
`validate(): Partial<Record<keyof IBuyer, string>>` - выполняет проверку данных покупателя и возвращает объект с ошибками валидации.

### Слой коммуникации

#### Класс AppApi

Класс отвечает за взаимодействие приложения с сервером. Класс использует композицию и принимает экземпляр класса Api, через который выполняются HTTP-запросы.
...
`createOrder(order: IOrder): Promise<IOrderResult>` - выполняет POST запрос на эндпоинт `/order/`, отправляет данные заказа и возвращает объект с подтверждением заказа.